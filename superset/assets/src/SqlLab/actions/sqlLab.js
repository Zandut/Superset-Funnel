...
export function estimateQueryCost(query) {
  ...
  const endpoint = schema === null
      ? `/metrix/estimate_query_cost/${dbId}/`
      : `/metrix/estimate_query_cost/${dbId}/${schema}/`;
...
export function fetchQueryResults(query) {
  return function (dispatch) {
    ...
    return SupersetClient.get({
      endpoint: `/metrix/results/${query.resultsKey}/`,
      parseMethod: 'text',
    })
      .then(({ text = '{}' }) => {
...
export function validateQuery(query) {
  return function (dispatch) {
    ...
    return SupersetClient.post({
      endpoint: `/metrix/validate_sql_json/${window.location.search}`,
      postPayload,
      stringify: false,
    })
      .then(({ json }) => {
...
export function addTable(query, tableName, schemaName) {
  return function (dispatch) {
    ...
    SupersetClient.get({ endpoint: encodeURI(`/metrix/table/${query.dbId}/` +
    ...
    SupersetClient.get({
      endpoint: encodeURI(`/metrix/extra_table_metadata/${query.dbId}/` +
          `${encodeURIComponent(tableName)}/${encodeURIComponent(schemaName)}/`),
    })
      .then(({ json }) =>
...
export function popDatasourceQuery(datasourceKey, sql) {
  return function (dispatch) {
    return SupersetClient.get({
      endpoint: `/metrix/fetch_datasource_metadata?datasourceKey=${datasourceKey}`,
    })
      .then(({ json }) =>
        dispatch(
          addQueryEditor({
